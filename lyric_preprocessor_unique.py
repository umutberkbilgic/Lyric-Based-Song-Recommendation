...
print("Lyric pre-processor (unique) start ...")
...
with open("lyrics.csv", "rt", encoding="utf8") as csvfile:
    ...
    for row in wsreader:
        if flag:
            ...
            # Uniquely tokenize on space characters and remove stop words
            used = set()
            lyric_tokens = lyric.split(" ")
            unique_tokens = [x for x in lyric_tokens if x not in used and (used.add(x) or True)]            
...
with open("lyrics_preproc_unique.csv", "w+", encoding="utf8") as new_csv:
    csvWriter = csv.writer(new_csv, delimiter=',')
    csvWriter.writerows(dataset)
    
print("  ! End of writing to .csv file.")
print("End of lyric pre-processor (unique).")
